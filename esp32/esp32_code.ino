/*
 * ESP32 Cold Storage AI -- Sensor Node (Lightweight)
 * ===================================================
 * Architecture: ESP32 reads sensors → HTTP POST to FastAPI → reads prediction response
 *
 * TinyML was removed from ESP32. Inference runs on the FastAPI backend.
 * This reduces flash usage from 102% → ~15%, fixing the "Sketch too big" error.
 *
 * Libraries required (install via Arduino Library Manager):
 *   - DHT sensor library  (by Adafruit)
 *   - Adafruit Unified Sensor
 * Built-in (no install needed):
 *   - WiFi.h, HTTPClient.h  (part of ESP32 Arduino core)
 */

#include <WiFi.h>
#include <HTTPClient.h>
#include <DHT.h>
#include <ArduinoJson.h>   // Install: "ArduinoJson" by Benoit Blanchon (~75KB)

// ─── CONFIGURATION (change before flashing) ──────────────────────────────────
#define WIFI_SSID        "YOUR_WIFI_SSID"
#define WIFI_PASSWORD    "YOUR_WIFI_PASSWORD"
#define API_URL          "http://YOUR_SERVER_IP:8000/api/ingest"
#define API_KEY          "super-secret-edge-key-2026"
#define POST_INTERVAL_MS 2000   // Send every 2 seconds
